...
function advanceWakeFlood(game) {
  ...
  if (game._wakePlayerTurns >= 6) {
    nextLevel = 2;
  } else if (game._wakePlayerTurns >= 3) {
    nextLevel = 1;
  }
...
export function attack(game, side, attackerUid, target) {
  ...
  if (totalBonus > 0 && dealtBonus < totalBonus && dealtBonus > 0) {
    // 남은 체력보다 보너스가 컸던 경우 실제 피해량만 Impact에 반영한다.
  }
