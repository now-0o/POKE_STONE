...
function getColumnCount(width) {
  ...
  return Math.max(2, Math.min(6, Math.floor((usable + 8) / 142)));
}

export default function MobileDeckEditorV2({ save, onSaveChange, onBack }) {
  ...
  useEffect(() => {
    ...
    return () => document.body.classList.remove("mobile-deck-editing");
  }, []);
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      setColumns(getColumnCount(pane.clientWidth));
      setViewportHeight(Math.max(1, pane.clientHeight));
    };
  ...
  const endIndex = Math.min(ownedCards.length, endRow * columns);
  const visibleCards = ownedCards.slice(startIndex, endIndex);
  ...
  return (
    <div
      className="mobile-deck-editor mobile-deck-editor-v2"
