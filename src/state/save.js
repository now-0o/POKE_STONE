...
import {
  PACKS,
  CARDS, CARD_MAP, STARTER_DECK,
  PACK_PRICE, PACK_SIZE, RARITY_WEIGHTS, RARITY_REFUND, MAX_COPIES,
} from '../data/cards.js';
  ...
  try {
    ...
    if (raw) {
      ...
      (save.deck || []).forEach((id) => {
        ...
        if (!card) return;
        const limit = Math.min(MAX_COPIES[card.rarity], save.collection[id] || 0);
...
export function openPack(save, packId = 'basic') {
  ...
  const rarities = [];
  for (let i = 0; i < PACK_SIZE; i++) rarities.push(rollRarityFrom(pack.weights));
  // 보장 등급: 팩별 최소 1장 보장 (basic=레어+, premium=에픽+)
  const gIdx = RARITY_ORDER.indexOf(pack.guarantee);
  if (!rarities.some((r) => RARITY_ORDER.indexOf(r) >= gIdx)) rarities[PACK_SIZE - 1] = pack.guarantee;
...
export function deckIsValid(save) {
  ...
  for (const id of save.deck) {
    ...
    if (counts[id] > Math.min(MAX_COPIES[card.rarity], save.collection[id] || 0)) return false;
  }
  return true;
}
...
export function activateAdminMode(save) {
  ...
  CARDS.forEach((c) => {
    if (c.kind === 'pokemon' || c.kind === 'spell' || c.kind === 'item' || c.kind === 'mega') {
